...
public class Usercontroller {
    ...
    public Result doregister(@RequestBody Userinfo registerinfo){
        ...
        if(registerinfo.getName()==null) return result;
    ...
    public Result Editdata(@RequestBody Userinfo userinfo){
        ...
        try {
            ...
            if (userinfo.getPassword() .length()>5&&userinfo.getPassword() .length()<20) {
    ...
    public Result docollect(@RequestBody Collect collect) {
        ...
        LocalDateTime currentTime = LocalDateTime.now();
        try {
            if(jdbc.queryForObject("select count(*) from collect where collect.userid=? and collect.postid=?",Long.class,collect.getUserid(),collect.getPostid()).equals(0)){
    ...
    public Result personnum(@RequestBody Userinfo userinfo){
        ...
        try {
            ...
            Integer num4=jdbc.queryForObject("select count(*) from collect,post where post.userid=? and collect.postid=post.postid",Integer.class,userinfo.getUserid());
