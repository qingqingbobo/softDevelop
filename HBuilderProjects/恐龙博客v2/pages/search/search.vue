	...
	export default {
		...
		methods: {
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['前端', '后端', '日记', '数学', '数据库', '数据结构与算法', '服务器', '硬件', '移动开发'];
			},
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
				...
				uni.request({
					url: 'http://localhost:8080/search?key=' + keyword,
					method: "GET", //仅为示例
					// success: (res) => {
					...
					success: (res) => {
						...
						if (res.data.code * 1 == 200) {
							try {
								...
								console.log("搜索成功");
								uni.setStorageSync('postinput', res.data.result); //搜索到的内容存在postinput中
								console.log(res.data.result);
							...
							this.$u.toast('登陆失败，用户名或密码错误') //提示框
						}
					}
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.code * 1 == 200) {
							try {
								console.log("发送成功");
								uni.setStorageSync('postkeyword', res.data
									.result); //搜索到的文章信息存储在postkeyword中
								console.log(res.data.result);
								//保存用户信息  与后端对应
								uni.navigateTo({
									url: '../show/show'
								});
								//登陆成功后返回 我的个人信息页面 （退回上一个页面）
							} catch (e) {
								this.$u.toast('搜索异常')
							}
						} else {
							this.$u.toast('搜索失败') //提示框
						}
					}
