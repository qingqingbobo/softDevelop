<template>
	<view class="page" :model="post">
		<view class="feedback-body">
			<input v-model="post.title" placeholder="   请输入博客标题" />
			<textarea placeholder="请输入内容(强制HTML文件)" style="border-bottom:1px solid   #F1F1F3;"
				v-model="post.content" class="feedback-textare" />
		</view>
		...
		<view class="feedback-body">
			<input class="feedback-input" v-model="post.tags" placeholder="请输入标签" />
		</view>
		<!-- 类别 -->
		<view class='feedback-title'>
			<text>类别：</text>
		...
		<view class="feedback-body">
			<input class="feedback-input" v-model="post.categories" placeholder="请输入类别" />
		</view>

		<u-button type="success" shape="circle" ripple="true" class="feedback-submit" @tap=" publish">提交</u-button>
